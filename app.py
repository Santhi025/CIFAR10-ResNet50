...
st.markdown("""
<div class="hero">
<h1>🧠 CIFAR-10 Vision Lab Pro</h1>
<p>Upload an image → get prediction + Grad-CAM explainability + analytics</p>
</div>
""", unsafe_allow_html=True)
...
if uploaded_file:
    ...
    with col1:
        ...
        try:
            ...
            cam = overlay_heatmap(image, heatmap)
            st.image(cam, caption="Grad-CAM Explanation", use_container_width=True)
        except:
            st.warning("Grad-CAM not available for this model layer.")


    with col2:
        st.markdown('<div class="card">', unsafe_allow_html=True)

        st.markdown(f"<div class='result'>{CLASS_ICONS[pred_class]} {pred_class}</div>", unsafe_allow_html=True)
        st.progress(confidence)
        ...
        for i in top3:
            st.write(f"{CLASS_ICONS[CLASS_NAMES[i]]} {CLASS_NAMES[i]} — {preds[i]:.2%}")
